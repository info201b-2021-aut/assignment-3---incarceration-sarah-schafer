...
race_over_time <- data %>%
  select(year, total_pop_15to64, aapi_pop_15to64, black_pop_15to64, latinx_pop_15to64,
         ...
         native_jail_pop, white_jail_pop) %>% # selecting relevant columns
...
# Each line graph is set up in a similar way.  The "aspect.ratio = 1" term is to
# ensure that all of the charts are square.  
...
# Make a dataframe that contains national racial jail information in the year 2018
pie_jail_df <- total_jail_over_time %>%
  filter(year == 2018) %>%
  filter(Race != "total_jail_all") # filter out the total rows, as this will not be included in the pie chart
# Make a dataframe that contains national racial information about the general 
